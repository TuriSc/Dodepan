...
#ifdef USE_POTS
FilterTypeDef pot0_filtered;        // Moving average filters to smooth the
FilterTypeDef pot1_filtered;        // two analog inputs.
#endif
...
// Detuning affects the entire audio engine. By changing the PWM clock divider,
// we won't simply change pitch but also squash and stretch playback time.
...
#ifdef USE_POTS
void analog_io_task(){
    // Throttle this function as it's not time-critical
    static uint8_t throttle;
    if(throttle++ % 16 != 0) return;
    static uint16_t raw_reading;
    static uint16_t filtered_reading;

    // Scale potentiometer
    static uint8_t selected_scale; 
    static uint8_t previous_scale;
    adc_select_input(POT0_PIN - 26); // selecting ADC0
    raw_reading = adc_read(); // 0 to 0xfff (4095)
    filtered_reading = Moving_Average_Compute(raw_reading, &pot0_filtered);
    
    if (filtered_reading >= 0 && filtered_reading <= 0xfff){
        // To compensate for jittery readings, an extra value
        // is added at the end of the mapping scale,
        // then the result is capped to exclude the extra value.
        selected_scale = map(filtered_reading, 0, 0xfff, 0, num_scales);
        selected_scale = selected_scale < num_scales? selected_scale : num_scales - 1;
    }
    if (selected_scale != previous_scale){
        tuning.scale = selected_scale;
        update_scale();
    ...
    static uint8_t note;
    static bool alteration;
    
    adc_select_input(POT1_PIN - 26); // selecting ADC1
    raw_reading = adc_read();
    filtered_reading = Moving_Average_Compute(raw_reading, &pot1_filtered);
    if (filtered_reading >= 0 && filtered_reading <= 0xfff){
        selected_key = map(filtered_reading, 0, 0xfff, 0, NUM_ROOT_KEYS);  // Again, we only need it to span 0-23
                                                                // but we need an extra step at the end
                                                                // of the potentiometer range.
        selected_key = selected_key < NUM_ROOT_KEYS ? selected_key : NUM_ROOT_KEYS -1;   // Cap
    }
    if (selected_key != previous_key){
        tuning.key = selected_key; // Change key
...
int main() {
    ...
    #ifdef USE_POTS
    bi_decl(bi_1pin_with_name(POT0_PIN, POT0_PIN_DESCRIPTION));
    bi_decl(bi_1pin_with_name(POT1_PIN, POT1_PIN_DESCRIPTION));
    #endif
    #ifdef USE_BUTTONS
    ...
    audio_pin_slice = pwm_gpio_to_slice_num(SOUND_PIN);

    adc_init();
    #ifdef USE_POTS
    adc_gpio_init(POT0_PIN);
    adc_gpio_init(POT1_PIN);
    Moving_Average_Init(&pot0_filtered);
    Moving_Average_Init(&pot1_filtered);
    #endif
